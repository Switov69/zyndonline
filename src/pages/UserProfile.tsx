...
export default function UserProfile() {
  ...
  useEffect(() => {
    if (!id) return;
    getUserById(id).then((u) => { setProfileUser(u); setLoading(false); });
  }, [id, getUserById]);

  if (loading) return <div className="flex items-center justify-center py-20"><div className="w-8 h-8 border-2 border-accent-500/30 border-t-accent-500 rounded-full animate-spin" /></div>;

  if (!profileUser) return (
    <div className="text-center py-20">
      <h2 className="text-white text-xl font-semibold mb-2">Пользователь не найден</h2>
      <Link to="/" className="inline-flex items-center gap-2 px-5 py-2.5 bg-accent-500 hover:bg-accent-600 text-white rounded-xl text-sm font-medium transition-colors mt-4"><ArrowLeft size={16} />На главную</Link>
    </div>
  );
  ...
  if (currentUser && currentUser.id === profileUser.id) return (
    <div className="text-center py-20">
      ...
      <Link to="/profile" className="inline-flex items-center gap-2 px-5 py-2.5 bg-accent-500 hover:bg-accent-600 text-white rounded-xl text-sm font-medium transition-colors">Перейти в профиль</Link>
    </div>
  );
  ...
  const cardStyle: React.CSSProperties = profileBg
    ? profileBg.startsWith('linear') || profileBg.startsWith('radial') ? { background: profileBg } : { backgroundImage: `url(${profileBg})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : {};

  return (
    <div className="max-w-3xl mx-auto">
      <Link to="/" className="inline-flex items-center gap-2 text-dark-300 hover:text-white text-sm mb-6 transition-colors"><ArrowLeft size={16} />Назад</Link>
      <div className="border border-dark-700/50 rounded-2xl p-6 sm:p-8 mb-6 relative overflow-hidden" style={cardStyle}>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6">
          <div className="w-24 h-24 rounded-2xl bg-dark-600 flex items-center justify-center text-dark-200 text-3xl font-bold overflow-hidden border-2 border-dark-500/50 shrink-0">
            {profileUser.avatar ? <img src={profileUser.avatar} alt="" className="w-full h-full object-cover" /> : profileUser.username.charAt(0).toUpperCase()}
          </div>
          <div className="flex-1 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-2 mb-1 flex-wrap">
              <h1 className="text-2xl font-bold text-white">{profileUser.username}</h1>
              {hasPremium && <span className="inline-flex items-center gap-1 bg-purple-500/25 text-purple-300 border border-purple-500/40 px-2 py-0.5 rounded-md text-xs font-semibold"><Crown size={11} />Premium</span>}
              {profileUser.isAdmin && <span className="inline-flex items-center gap-1 bg-red-500/20 text-red-300 border border-red-500/30 px-2 py-0.5 rounded-md text-xs font-semibold">Admin</span>}
            </div>
            ...
            {profileUser.telegram && (
              <div className="flex items-center justify-center sm:justify-start gap-1.5 text-sm text-accent-400 mb-2">
                <Send size={13} />
                <a href={`https://t.me/${profileUser.telegram.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="hover:text-accent-300 transition-colors">{profileUser.telegram}</a>
              </div>
            )}
      ...
      <div className="flex gap-1 bg-dark-800/30 rounded-xl p-1 mb-6 border border-dark-700/30">
        {(['info','jobs'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === t ? 'bg-dark-700 text-white shadow-sm' : 'text-dark-400 hover:text-dark-200'}`}>
      ...
      {tab === 'info' && (
        <div className="bg-dark-800/50 border border-dark-700/50 rounded-2xl p-6 sm:p-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-dark-700/30 rounded-xl p-4"><p className="text-dark-400 text-xs mb-1">Подписка</p><p className="text-sm font-semibold">{hasPremium ? <span className="text-purple-400">Premium ✓</span> : <span className="text-dark-400">Нет</span>}</p></div>
          </div>
        </div>
      )}
      {tab === 'jobs' && (
        userJobs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">{userJobs.map(job => <JobCard key={job.id} job={job} showExecutor={false} />)}</div>
        ) : (
          <div className="text-center py-16"><Briefcase size={36} className="text-dark-600 mx-auto mb-4" /><h3 className="text-white font-semibold text-lg mb-1">Нет активных заказов</h3><p className="text-dark-400 text-sm">У пользователя нет открытых заказов</p></div>
        )
      )}
    </div>
  );
