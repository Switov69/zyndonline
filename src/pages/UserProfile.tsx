...
export default function UserProfile() {
  ...
  const hasPremium = profileUser.subscription?.active &&
    profileUser.subscription?.expiresAt &&
    new Date(profileUser.subscription.expiresAt) > new Date();

  const profileBg = profileUser.subscription?.profileBg;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <Link
        to="/"
        className="inline-flex items-center gap-2 text-dark-300 hover:text-white text-sm mb-6 transition-colors"
      >
        <ArrowLeft size={16} />
        Назад
      </Link>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-2 mb-1 flex-wrap">
              ...
              {profileUser.isAdmin && (
                <span className="inline-flex items-center gap-1 bg-red-500/20 text-red-300 border border-red-500/30 px-2 py-0.5 rounded-md text-xs font-semibold">
                  Admin
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm text-dark-300">
              <span className="flex items-center gap-1.5">
                ...
                С {profileUser.joinedAt}
              </span>
              <span className="flex items-center gap-1.5">
                <Briefcase size={14} />
                {userJobs.length} активных заказов
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-1 bg-dark-800/30 rounded-xl p-1 mb-6 border border-dark-700/30">
        <button
          ...
          className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
            tab === 'info' ? 'bg-dark-700 text-white shadow-sm' : 'text-dark-400 hover:text-dark-200'
          }`}
        >
          Информация
        </button>
        <button
          onClick={() => setTab('jobs')}
          className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
            tab === 'jobs' ? 'bg-dark-700 text-white shadow-sm' : 'text-dark-400 hover:text-dark-200'
          }`}
        >
          Заказы {profileUser.username} ({userJobs.length})
        </button>
      </div>
        ...
        <div className="bg-dark-800/50 border border-dark-700/50 rounded-2xl p-6 sm:p-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-dark-700/30 rounded-xl p-4">
              <p className="text-dark-400 text-xs mb-1">Активных заказов</p>
            ...
            <div className="bg-dark-700/30 rounded-xl p-4">
              ...
              <p className="text-white font-semibold text-sm">{profileUser.joinedAt}</p>
            </div>
            <div className="bg-dark-700/30 rounded-xl p-4">
              <p className="text-dark-400 text-xs mb-1">Подписка</p>
              <p className="text-sm font-semibold">
                {hasPremium ? (
                  <span className="text-purple-400">Premium ✓</span>
                ) : (
                  <span className="text-dark-400">Нет</span>
                )}
              </p>
            </div>
          </div>
      ...
      {tab === 'jobs' && (
        userJobs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            {userJobs.map((job) => (
              <JobCard key={job.id} job={job} showExecutor={false} />
            ))}
          </div>
        ) : (
          <div className="text-center py-16">
            ...
            <h3 className="text-white font-semibold text-lg mb-2">Нет активных заказов</h3>
            <p className="text-dark-400 text-sm">У этого пользователя пока нет открытых заказов</p>
          </div>
        )
      )}
    </div>
