...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (result === true) { navigate('/'); } else { setError(result as string); }
  };
    ...
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-dark-400 text-sm">Войдите в свой аккаунт</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-dark-800/50 border border-dark-700/50 rounded-2xl p-6 sm:p-8 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Ваш никнейм"
          ...
          <p className="text-center text-dark-400 text-sm">
            Нет аккаунта?{' '}
            <Link to="/register" className="text-accent-400 hover:text-accent-300 transition-colors">Зарегистрироваться</Link>
          </p>
        </form>
      </div>
    </div>
